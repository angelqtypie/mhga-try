...
const CenterSchedulesPage: React.FC = () => {
  ...
  useEffect(() => {
    const scheduleEvents = schedules.map((item, index) => ({
      id: `static-${index}`,
      title: `${item.activity} (${item.status})`,
      start: item.date,
      allDay: true,
      backgroundColor: item.status === 'Available' ? '#bbf7d0' : '#fecaca',
      borderColor: item.status === 'Available' ? '#22c55e' : '#ef4444',
      textColor: '#000'
    }));
    setEvents(prev => [...scheduleEvents, ...prev]);
  }, []);
  ...
  const handleDateSelect = (selectInfo: any) => {
    const title = prompt("Enter activity title (for trial only, not saved):");
  ...
  const handleEventRemove = (clickInfo: any) => {
    if (confirm(`Remove event '${clickInfo.event.title}'? (Trial only)`)) {
    ...
    <div style={{ fontFamily: "Arial, sans-serif" }}>
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <main style={{ flex: 1, background: "#f1f5f9", overflowY: "auto", padding: "24px", maxHeight: "100vh" }}>
          <div style={{ backgroundColor: "#fff", padding: "24px", borderRadius: "10px", boxShadow: "0 1px 6px rgba(0,0,0,0.05)", overflowY: "auto", maxHeight: "calc(100vh - 48px)" }}>
            ...
            <h3 style={{ fontSize: "18px", marginBottom: "8px" }}>📅 Monthly View (Trial + Schedules)</h3>
            <div style={{ overflowX: "auto" }}>
              <FullCalendar
